...
async function initApp(){
  ...
  // Mostra subito la lista con le città salvate (senza aspettare la geo)
  // così l'utente vede qualcosa immediatamente
  ...
  // Prova geolocation con timeout breve
  let posCity=placeholder;
  try{
    // Race tra geo e timeout di 7s
    const pos=await Promise.race([
      getPosition(),
      new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),7000)),
    ]);
    // Ottieni il nome della città in background (senza bloccare)
    ...
    // Reverse geocoding in background — aggiorna il nome quando arriva
    reverseGeocode(pos.coords.latitude,pos.coords.longitude).then(geo=>{
      ...
      // Aggiorna la card e l'header se siamo già nella detail
      renderCityList();
      $('d-city').textContent=geo.city;
      $('d-sub').textContent=[geo.region,geo.country].filter(Boolean).join(' · ');
    }).catch(()=>{});

  }catch{
    // Fallback silenzioso: usa Milano come posizione
    posCity={city:'Milano',country:'IT',region:'Lombardia',lat:45.4642,lon:9.1900,isPos:true};
    state.cities=[posCity,...saved];
    renderCityList();
  }

  // Carica il meteo di tutte le città e apri il dettaglio
  loadAllCityWeather();
