...
async function openDetailForCity(c){
  ...
  if(cached){
    $('d-city').textContent=c.city||'—';
    $('d-sub').textContent=[c.region,c.country].filter(Boolean).join(' · ');
    $('d-temp').textContent=Math.round(cached.temp);
    $('d-desc').textContent=wmo(cached.code).i+' '+wmo(cached.code).d;
    $('d-range').textContent=`Max ${Math.round(cached.hi)}° · Min ${Math.round(cached.lo)}°`;
  }

  // Loading overlay con timeout massimo di 8s — poi sparisce comunque
  setLoading('Caricamento meteo…');
  const loadingKill=setTimeout(()=>hideLoading(),8000);
  ...
  try{
    ...
    clearTimeout(loadingKill);
    hideLoading(); // Nascondi comunque — mostra quel che c'è
    // Messaggio di errore leggero solo se non abbiamo dati cached
    if(!state.weatherData){
      $('d-city').textContent=c.city||'—';
      $('d-sub').textContent='Connessione non disponibile';
      $('d-temp').textContent='—';
      $('d-desc').textContent='Riprova tra qualche secondo';
      $('d-range').textContent='';
    }
  }
}
...
function getPosition(){
  return new Promise((resolve,reject)=>{
    ...
    // 6s timeout — se non risponde usiamo fallback
...
async function initApp(){
  ...
  // Nascondi subito il loading — la UI si mostra immediatamente
  hideLoading();

  const saved=loadCitiesFromLS();
  const placeholder={city:'La mia posizione',country:'',region:'',lat:45.4642,lon:9.1900,isPos:true};
  state.cities=[placeholder,...saved];
  showView('list');
  renderCityList();

  // Geolocation in background — non blocca nulla
  let posCity=placeholder;
  try{
    const pos=await Promise.race([
      getPosition(),
      new Promise((_,rej)=>setTimeout(()=>rej(new Error('timeout')),6000)),
    ]);
    posCity={city:'La mia posizione',country:'',region:'',lat:pos.coords.latitude,lon:pos.coords.longitude,isPos:true};
    state.cities=[posCity,...saved];
    renderCityList();

    // Nome città in background
    reverseGeocode(pos.coords.latitude,pos.coords.longitude).then(geo=>{
      posCity.city=geo.city;
      posCity.country=geo.country;
      posCity.region=geo.region;
      state.currentCity=geo.city;
      state.currentCountry=geo.country;
      state.currentRegion=geo.region;
      renderCityList();
      if($('d-city'))$('d-city').textContent=geo.city;
      if($('d-sub'))$('d-sub').textContent=[geo.region,geo.country].filter(Boolean).join(' · ');
    }).catch(()=>{});
  }catch{
...
/* ── PTR (Pull to refresh) ── */
const detailScroll=$('view-detail');  // view-detail IS the scroll container now
let ptrStartY=0,ptrStartX=0,ptrActive=false,ptrIntent=null;
...
detailScroll.addEventListener('touchmove',e=>{
  ...
  if(!ptrIntent){
    if(dx>8)ptrIntent='h';
    else if(dy>6)ptrIntent='v';
  }
  if(ptrIntent==='h'){ptrActive=false;return;}
  if(ptrIntent==='v'&&!ptrActive&&dy>120){
    ptrActive=true;
...
detailScroll.addEventListener('touchend',()=>{
  if(ptrActive){
    ptrActive=false;ptrIntent=null;
...
/* ── HOURLY SCROLL touch fix ── */
function fixHourlyScroll(){
  const card=document.querySelector('#hourly-scroll')?.closest('.dcard');
  if(card){card.style.overflow='visible';card.style.overflowX='visible';}
}
...
function initMobileNav(){
  ...
  const hArrows =$('hourly-arrows');
  const strip   =$('hourly-scroll');
  const detView =$('view-detail');   // ← real scroll container
  const btnUp   =$('mnav-up');
  const btnDown =$('mnav-down');
  const btnLeft =$('mnav-left');
  const btnRight=$('mnav-right');
  const VSTEP   =Math.round(window.innerHeight*0.60);
  const HSTEP   =200;

  nav.classList.add('visible');

  btnUp.addEventListener('click',  ()=>detView.scrollBy({top:-VSTEP,behavior:'smooth'}));
  btnDown.addEventListener('click',()=>detView.scrollBy({top: VSTEP,behavior:'smooth'}));

  function refreshV(){
    btnUp.classList.toggle('dim',  detView.scrollTop<10);
    btnDown.classList.toggle('dim',detView.scrollTop+detView.clientHeight>=detView.scrollHeight-10);
  }
  detView.addEventListener('scroll',refreshV,{passive:true});

  btnLeft.addEventListener('click', ()=>strip.scrollBy({left:-HSTEP,behavior:'smooth'}));
  btnRight.addEventListener('click',()=>strip.scrollBy({left: HSTEP,behavior:'smooth'}));

  function refreshH(){
    btnLeft.classList.toggle('dim', strip.scrollLeft<10);
    btnRight.classList.toggle('dim',strip.scrollLeft+strip.clientWidth>=strip.scrollWidth-10);
  }
  strip.addEventListener('scroll',refreshH,{passive:true});

  const hourlyCard=strip.closest('.dcard');
  if(hourlyCard){
    new IntersectionObserver(entries=>{
      hArrows.classList.toggle('show',entries[0].isIntersecting);
      refreshH();
    },{threshold:0.2}).observe(hourlyCard);
  }
  ...
  window.addEventListener('weatherRendered',()=>setTimeout(()=>{refreshV();refreshH();},150));
}
