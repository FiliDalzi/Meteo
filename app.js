...
$('inst-btn').addEventListener('click',async()=>{
...
detailScroll.addEventListener('touchstart',e=>{if(detailScroll.scrollTop===0)ptrStart=e.touches[0].clientY;},{passive:true});
detailScroll.addEventListener('touchmove',e=>{if(detailScroll.scrollTop===0&&!ptrActive&&e.touches[0].clientY-ptrStart>65){ptrActive=true;$('ptr-ind').classList.add('show');}},{passive:true});
detailScroll.addEventListener('touchend',()=>{if(ptrActive){ptrActive=false;$('ptr-ind').classList.remove('show');if(state.currentLat)openDetailForCity({city:state.currentCity,country:state.currentCountry,region:state.currentRegion,lat:state.currentLat,lon:state.currentLon});}},{passive:true});
